...
`1.0.4` 是 `1.0.3` 之后的 `1.0.4-SNAPSHOT` 发布候选准备草稿。本轮不切正式版本、不打 Git tag、不上传 Maven Central；当前稳定版本仍是 `1.0.3`。

本草稿已经包含两类内容：

- `1.0.4` 发版前强化：安全边界、SSE 解析鲁棒性、Testcontainers 兼容性、测试隔离和示例安全表达。
- 企业级能力增强：MyBatis starter、服务层业务幂等、字段级 AES-GCM 加密和响应脱敏。
...
- Maven Central：本草稿不发布，`1.0.3` 仍是当前稳定版本。
...
mvn -B -ntp -U -gs docs/central-dry-run-settings.xml -s docs/central-dry-run-settings.xml \
  -Papi-compat \
...
Docker/Testcontainers 只在测试服务器执行：

```bash
mvn -B -ntp -Pintegration-tests -pl under-utils-test -am test
```

远端验证后应删除临时工作目录、测试日志，并清理 Docker 镜像、容器、卷缓存，避免测试服务器磁盘持续膨胀。

## 发布前注意

- 真正发布 `1.0.4` 前，需要将 Maven reactor 版本从 `1.0.4-SNAPSHOT` 收敛为 `1.0.4`。
- 发布提交需要重新执行本文件的验证计划，并把实际验证时间和结果写回本文件。
- 发布时再创建 Git tag `v1.0.4`、GitHub Release，并通过 Maven Central 工作流上传构件。
